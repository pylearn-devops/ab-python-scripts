...
# Define the ignored date (e.g., September 30, 2023)
...
def close_pulls_and_issues(org_name):
    ...
    for repo in repos:
        # Close open pull requests older than a year and before the ignored date
        pull_requests = repo.get_pulls(state='open')
        for pr in pull_requests:
            if closed_count >= max_to_close:
                break  # Stop processing if we've reached the limit
            if cutoff_date <= pr.created_at < ignored_date:  # Only consider PRs created within the specified date range
        ...
        # Close open issues older than a year and before the ignored date
        issues = repo.get_issues(state='open')
        for issue in issues:
            if closed_count >= max_to_close:
                break  # Stop processing if we've reached the limit
            if cutoff_date <= issue.created_at < ignored_date and issue.pull_request is None:  # Only consider issues created within the specified date range
