...
ignored_date = datetime(2024, 6, 27, tzinfo=timezone.utc)
            ...
            if issue.created_at < ignored_date and issue.pull_request is None:  # Only consider issues created before the ignored date
                if has_exempt_labels(issue):
                    ...
                    closed_issues.append(
                        [issue.number, issue.title, issue.user.login, issue.created_at, issue.closed_at, repo.name])
                    closed_count += 1
...
def close_pulls_and_issues_for_orgs():
    ...
    # if all_closed_prs:
    #     save_to_csv("closed_prs.csv", all_closed_prs,
    #                 ["PR Number", "Title", "User", "Created At", "Closed At", "Repository"])
    # if all_closed_issues:
    #     save_to_csv("closed_issues.csv", all_closed_issues,
    #                 ["Issue Number", "Title", "User", "Created At", "Closed At", "Repository"])
