...
def close_pulls_and_issues(org_name):
    ...
    for repo in repos:
        ...
        for pr in pull_requests:
            if closed_count >= max_to_close:
                break
            if pr.created_at < cutoff_date and not has_exempt_labels(pr):  # Check if PR is older than cutoff date and has no exempt labels
        ...
        for issue in issues:
            if closed_count >= max_to_close:
                break
            if issue.created_at < cutoff_date and issue.pull_request is None and not has_exempt_labels(issue):  # Check if issue is older than cutoff date and has no exempt labels
    ...
    return closed_prs, closed_issues
...
def close_pulls_and_issues_for_orgs():
    ...
    for org in ORGANIZATIONS:
        closed_prs, closed_issues = close_pulls_and_issues(org)
    ...
    print(f"Total closed PRs: {len(all_closed_prs)}")
